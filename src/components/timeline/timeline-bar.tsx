...
interface TimelineBarProps {
  character: (null | CharacterConstants);
      ...
      {skillSequence.length > 0 && (
        <div className="relative h-12 bg-muted/20 border border-muted/30 rounded-lg">
          {skillSequence.map((skill, index) => {
            if (character && character.name.toLowerCase() === skill.source)
              return (
                <div
                  key={`${skill.id}-${index}`}
                  title={`${skill.type}: ${skill.name}`}
                  className={`absolute top-0 h-full ${skill.bgColor} ${skill.textColor} rounded-sm flex items-center justify-between text-xs font-medium border-l border-gray-100 group cursor-pointer transition-all hover:brightness-110`}
                  style={{
